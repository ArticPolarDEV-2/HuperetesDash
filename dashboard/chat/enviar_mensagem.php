...
if (isset($_POST['mensagem']) && !empty($_POST['mensagem'])) {
    ...
    try {
        ...
        $userType = isset($_SESSION["adm_id"]) ? 'admin' : 'user';
        $userId   = $_SESSION["adm_id"] ?? $_SESSION["user_id"];

        $stmt = $db->prepare("INSERT INTO chat_messages (user_id, user_type, message, sent_in) VALUES (?, ?, ?, NOW())");
        $stmt->execute([$userId, $userType, $mensagem]);
